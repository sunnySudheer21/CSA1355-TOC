...
// Define the DFA
typedef struct {
    int transitions[3][2]; // [current state][input 'a' or 'b']
    int start_state;
    int accepting_state;
} DFA;

// Initialize the DFA for the language starting with 'a' and ending with 'b'
void initialize_dfa(DFA *dfa) {
    ...
    dfa->transitions[1][1] = 2;  // State 1 on input 'b'
    dfa->transitions[2][0] = -1; // State 2 on input 'a'
    dfa->transitions[2][1] = -1; // State 2 on input 'b'

    dfa->start_state = 0; // Initial state
    dfa->accepting_state = 2; // Accepting state
}
