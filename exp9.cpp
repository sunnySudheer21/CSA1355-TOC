...
void initialize_nfa(NFA *nfa) {
    ...
    // Add transitions
    // Transition for state 0 on 'b' (represented by 1)
    nfa->transitions[0][1][0] = 1;

    // Transitions for state 1 on 'a' and 'b'
    nfa->transitions[1][0][0] = 1; // 'a' (represented by 0)
    nfa->transitions[1][1][0] = 1; // 'b' (represented by 1)
    nfa->transitions[1][0][1] = 2; // 'a' leading to accepting state 2
}
